...
def test_can_selling_reduce_discretized_vonorm_list(monoclinic_lattice):
    ...
    discretized = DiscretizedVonormComputer(vonorm_list, lattice_step_size=0.05).find_closest_valid_vonorms()
...
def test_selling_reductions_equivalent(lattice):
    ...
    assert reduced_sb.compute_vonorms().is_superbasis()
    assert reduced_vonorms.is_superbasis()
