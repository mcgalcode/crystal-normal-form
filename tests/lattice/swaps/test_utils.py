import cnf.lattice.swaps.utils as swap_utils
import numpy as np
...
def test_get_unimodular_matrix_for_swap():
    ...
    # to swap
    swap = (0, 1)
    mat = swap_utils.get_unimodular_matrix_for_swap(swap)
...
def test_apply_swap():
    ...
    swap = (2, 3)
    swap_utils.apply_swap(label_list, swap)
...
def test_apply_swap_series():
    ...
    swap_utils.apply_swaps(label_list, swap_series)
    assert label_list == [0, 3, 1, 6, 5, 4, 2]
...
def test_get_unimodular_matrix_for_swap_series():
    swap_series = [(2, 3), (3, 6), (1, 2), (4, 5)]
    mat = swap_utils.get_unimodular_matrix_for_swap_series(swap_series)
    assert (mat.T[0] == [0, 0, 1]).all()
